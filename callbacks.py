...
def process_trading_desk(all_on, all_off, buy, sell, heartbeat, sym, qty, px, filter_text, chart_store):
    ...
    if not isinstance(chart_store, dict) or "times" not in chart_store:
        chart_store = {"times": [], "dollars": [], "percentages": []}
    ...
    mock_pct_val = (aggregated_cumu_pnl / 100000.0) * 100.0 if aggregated_cumu_pnl != 0 else 0.0
    chart_store["percentages"].append(mock_pct_val)
    chart_store["times"] = chart_store["times"][-100:]
    chart_store["dollars"] = chart_store["dollars"][-100:]
    chart_store["percentages"] = chart_store["percentages"][-100:]
    ...
    percent_fig = {
        "data": [{"x": chart_store["times"], "y": chart_store["percentages"], "type": "scatter", "mode": "lines",
    ...
    total_pnl_text = f"${aggregated_cumu_pnl:+,.2f}"
    total_pnl_style={"fontSize": "2rem", "fontWeight": "700", "fontFamily": SANS, "color": TEAL if aggregated_cumu_pnl >= 0 else RED}
